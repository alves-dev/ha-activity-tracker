...
Keep intermediate monitor data and options in flow-local dictionaries. Each step validates only its own inputs and advances on success. Validate cross-field requirements before moving forward, then use a final review step before the entry is created or updated.
